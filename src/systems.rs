...
use crate::components::{
    CelestialBody, OrbitChild, OrbitParent, PlayerCamera, PlayerVehicle, TeamRedHomePlanet,
};
...
pub fn move_player(
    ...
    external_force.force = movement.y
        * player.speed
        * modifier
        * Vec2::new(movement_direction.x, movement_direction.y).normalize();
...
pub fn orbit_objects(
    mut orbit_children: Query<(&mut Velocity, &Transform, With<OrbitChild>)>,
    orbit_parents: Query<(&Transform, With<OrbitParent>, Without<OrbitChild>)>,
    time: Res<Time>,
) {
    for mut orbit_child in orbit_children.iter_mut() {
        for orbit_parent in orbit_parents.iter() {
            ...
            let gravity = calculate_gravity(
                child_transform.translation,
                orbit_parent.0.translation,
                1.0,
                1_000_000.0,
            );
...
pub fn draw_player_orbit(
    player_query: Query<(&PlayerVehicle, &Transform, &Velocity, With<OrbitChild>)>,
    orbit_parents: Query<(&Transform, With<OrbitParent>, Without<OrbitChild>)>,
    mut gizmos: Gizmos,
    time: Res<Time>,
) {
    ...
    'outer: for _ in 0..10_000 {
        ...
        let next_position = current_position
            + Vec3::new(current_velocity.x, current_velocity.y, 0.0) * time.delta_seconds();
        gizmos.line_2d(
            Vec2::new(current_position.x, current_position.y),
            Vec2::new(next_position.x, next_position.y),
            Color::WHITE,
        );
